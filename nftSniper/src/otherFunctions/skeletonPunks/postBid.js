...
function send(
	...
	let tokenListedPrice = 0;
	let tokenUSTPrice = 0;
	let tokenLunaPrice = 0;
	let tokenId = data[0].events[6].attributes[5].value;
	snekfetch
		.get(
			...
			for (const [key, value] of Object.entries(response.body.prices)) {
				if (value.tokenId === tokenId) {
					if (value.sellPriceCurrency === 'LUNA') {
						tokenListedPrice = Number(value.sellPriceAmount);
					} else {
						tokenListedPrice = Number(
							(value.sellPriceAmount / lunaPrice).toFixed(2)
						);
					}
					if (data[0].events[6].attributes[7].value === 'uluna') {
						tokenLunaPrice = convertPrice(
							data[0].events[6].attributes[6].value
						);
						tokenUSTPrice = convertPrice(
							data[0].events[6].attributes[6].value * lunaPrice
						);
					} else if (
						data[0].events[6].attributes[7].value === 'uusd'
					) {
						tokenUSTPrice = convertPrice(
							data[0].events[6].attributes[6].value
						);
																		...
																		if (
																			...
																			if (
																				...
																				await sendChannel.send(
																					{
																						content: `https://marketplace.luart.io/collections/${collectionAddress}/${tokenId}`,
																						embeds: [
																							embed,
																						],
																					}
